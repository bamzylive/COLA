    ...
    const uint16_t  d)
{
    ...
    memset(f, 0, sizeof(int16_t) * N);  // string.h
    count = 0;
    while(count < d+1)
    {
...
{
    int16_t degree = -1;  // -1 represents zero polynomial
    for (int i =  N-1; i > -1; i--){
        if(f[i] !=0 ){
          degree = i;
          break;
...
int16_t  EEA(
  ...
  int16_t* pt)
{
    int16_t r0,r1,r2,s0,s1,s2,t0,t1,t2,q2;
    r0 = a;
    r1 = b;
    s0 = 1;
    s1 = 0;
...
void poly_div(
 ...
 )
 {
    if(p == 2){
      ...
      int i,j;
      int16_t u = num_inv(b[b_deg],p); // inv of leading coef of b mod p
      int16_t r_deg, d;
      int16_t v[N];
    ...
    //   display(b);
      ...
      r_deg = a_deg;
      //printf("initially degree of r is %hd\n", r_deg);
      //printf("initially degree of b is %hd\n", b_deg);
      while(r_deg >= b_deg){
          //printf("current degree of r is %hd\n", r_deg);
          //printf("current degree of b is %hd\n", b_deg);
          d = r_deg;
          v[d - b_deg] = (u * r[d]) % p ;
          for ( j = r_deg; j >= 0; j-- ){
...
)
{
    ...
    int16_t a_deg = dg(a);
    int16_t d,n;
    printf("degree of a is %hd\n",a_deg);
    printf("degree of b is %hd\n",b_deg );
    if ( b_deg * a_deg < 0){
        printf("here\n" );
        return;
    }
    ...
    c[ 2*n - 2 ] = dd[n-1];
    int16_t sum1, sum2;
    for (int16_t i = 1; i < 2*n-2; i ++){
        sum1 = 0;
        sum2 = 0;
        for(int16_t p = 0; p <= (i-1)/2; p++){
            sum1 += ( dd[ p ] + dd[ i-p ] );
...
)
{
    ...
    a_deg = dg(a);
    b_deg = dg(b);
    memset(olds,0,N*sizeof(int16_t));
    memset(oldt,0,N*sizeof(int16_t));
    memset(oldr,0,N*sizeof(int16_t));
    ...
    int16_t quo[N], prov[N],rem[N],temp[N];
    memset(quo, 0, N*sizeof(int16_t));
    
    while( dg(r) != -1 ){
        memset(rem, 0, N*sizeof(int16_t));


        poly_div(p,oldr,r,quo,rem,dg(oldr),dg(r));
        // quo = oldr div r

        memset(prov, 0, N*sizeof(int16_t));        
        poly_assign(prov, r); textbook_mul(temp, quo, r);
        poly_minus(r, oldr, temp); entrywise_mod_p(r, p);
        poly_assign(oldr, prov);
        // (oldr, r) := (r, old_r - quotient*r)
        
        memset(prov, 0, N*sizeof(int16_t));
        poly_assign(prov, s); textbook_mul(temp, quo, s);
        poly_minus(s, olds, temp); entrywise_mod_p(s, p);
        poly_assign(olds, prov);
        // (olds, s) := (s, old_s - quotient*s)

        memset(prov, 0, N*sizeof(int16_t));
        poly_assign(prov, t); textbook_mul(temp, quo, t);
        poly_minus(t, oldt, temp); entrywise_mod_p(t, p);
...
int quotient_ring_inv(
    ...
    int16_t n)
{
    // printf("Z_%hu[X] / X^%hd - 1\n",p,n);
    int16_t sum;
    int16_t a_deg = dg(a);    


    memset(a_inv, 0, N * sizeof(int16_t));
    ...
    int16_t minus_one[N], u[N], v[N], gcd[N];
    memset(minus_one, 0, N*sizeof(int16_t));
    ...
    if (dg(gcd) == 0){
        ...
        int16_t gcd_inv = num_inv(gcd[0],p);
